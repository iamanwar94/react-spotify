...
const id = "7wxfWZChsusJtGtBA4Ul3C"
...
const Tracks = ({token ,setToken}) => {
  ...
  const saveTrack = async (uri) => {
    ...
    const {data} = await axios.post(`https://api.spotify.com/v1/playlists/${id}/tracks`, JSON.stringify(formData),{
        headers: {
            Authorization: `Bearer ${token}`,
  ...
  const deleteTrack = async (uri) => {
    console.log("id", id, "uri", uri, "token", token)
    let formData = {tracks: [{uri:uri}]}
    const {data} = await axios.delete(`https://api.spotify.com/v1/playlists/${id}/tracks`, {
        headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
        },
